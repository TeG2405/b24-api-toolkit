...
describe("list-batched tests", () => {
  it.each([
    { total: 150, listSize: 50, batch: 1 }, { total: 155, listSize: 50, batch: 1 },
    { total: 10, listSize: 50, batch: 50 }, { total: 20, listSize: 20, batch: 20 },
  ...
  ("batched total: $total, listSize: listSize, batch: $batch", async ({ total, listSize, batch }) => {
    ...
    nock(process.env.WEBHOOK_URL || "").post('/crm.lead.list', { start: 0 }).reply(CODES.OK, {
    ...
    forEach(range(listSize, total, listSize * batch), (batchStart) => {
      ...
      nock(process.env.WEBHOOK_URL || "").post('/batch', { halt: true, cmd: commands }).reply(CODES.OK, {
        result: {
          result: results,
          result_error: [],
          result_total: [],
    ...
    const api = useApi();
    const res = await api.listBatched({ request: { method: "crm.lead.list" }, listSize: listSize, batchSize: batch });
