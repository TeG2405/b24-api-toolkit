...
const useHelpers = () => {
  const throwError = (error: ResponseError) => {
    throw new Error(
      `${error.error}: ${error.error_description || "no description"}`,
    );
  };
  ...
  const shouldCallRetry = ({
    ...
    return (
      isResponseError(body) &&
      includes(config.retry.errors, snakeCase(body.error))
    );
  };
  ...
  const getListResult = (result: unknown) => {
    if (!Array.isArray(result) && !isPlainObject(result))
      throw new Error(
        `Expecting 'result' to be a 'list' or a 'dict'. Got: ${result}`,
      );
    ...
    if (size(clues) !== 1)
      throw new Error(
        `If 'result' is a 'dict', expecting single item. Got: ${result}`,
      );
    const value = get(result, String(first(clues)));
    if (!Array.isArray(value))
      throw new TypeError(
        `If 'result' is a 'dict', expecting single item to be a 'list'. Got: ${result}`,
      );
    return value;
  };
  ...
  const getTail = ({
    ...
    if (response.next && response.next != listSize)
      throw new Error(
        `Expecting list chunk size to be ${listSize}}. Got: ${response.next}`,
      );
