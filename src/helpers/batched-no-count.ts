...
const useBatchedNoCount = ({
  ...
  if (request.parameters && request.parameters.order)
    throw new Error(
      "Ordering parameters are reserved in `listBatchedNoCount` method.",
    );
  const idFrom = `>${idKey}`;
  const idTo = `<${idKey}`;
  if (filter && (has(filter, idFrom) || has(filter, idTo)))
    throw new Error(
      `Filter parameters "${idFrom}" and "${idTo}" are reserved in "listBatchedNoCount" method.`,
    );
  ...
  const bodyRequests = ({
    ...
    headResult: ResponseSuccess["result"];
    tailResult: ResponseSuccess["result"];
  }) => {
    ...
    if (maxHeadId && minTailId && maxHeadId < minTailId) {
      forEach(range(maxHeadId, minTailId, listSize), (start) => {
        ...
        set(
          bodyRequest,
          ["parameters", "filter", idTo],
          min([start + listSize + 1, minTailId]),
        );
