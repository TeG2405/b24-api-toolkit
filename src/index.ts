...
const useApi = () => {
  ...
  const listBatchedNoCount = async ({
    ...
    const batchSizeTotal = batchSize || config.batchSize;
    const result: unknown[] = [];
    const batchedHelper = useBatchedNoCount({
      request,
      idKey,
  ...
  const updatesBatch = async ({
    ...
    return {
      bodyResults: result,
      headRequests: referenceHelper.headRequests({
        bodyRequests: concat(headRequests, bodyRequests),
        bodyResults: result,
      }),
    };
  };

  const referenceBatchedNoCount = async ({
    request,
    updates,
    idKey = "ID",
    listSize,
    ...
    const batchSizeTotal = batchSize || config.batchSize;
    const result: unknown[] = [];
    const referenceHelper = useReferenceNoCount({
      request,
      updates,
      idKey,
      listSize: listSizeTotal,
