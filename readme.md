# b24api — README 
Копия клиента для Б24 от https://github.com/shkarupa-alex/b24api/blob/master/b24api/async_api.py

Использовать его мы будем в N8N-узлах.

## Почему он такой
В б24 разные списочные методы работают сильно по-разному (постраничность, возможность выбирать батчами и фильтровать)
Из-за этого помимо двух основных методов которые дает апи (call, batch) есть еще и несколько методов для выборки списков

Любой внешний ресурс нестабилен - поэтому в клиент зашиты retry (они же помогают когда б24 говорит "слишком много запросов")

Б24 может выдавать не-200 статус и при этом в теле запроса присылать развернутое сообщение об ошибке. Поэтому порядок обработки ответа важно сохранить как в оригинальном клиенте.
